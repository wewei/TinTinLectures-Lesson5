...
export const withIdentity = (identity: Identity) =>
  cached((cid: string) => createActor(cid, { agentOptions: { identity } }));

export type MicroblogActor = ReturnType<typeof createActor>;

export const obIdentity = encapsulated(nothing as Maybe<Identity>)(
  ({ notify }) => ({ set: notify })
);

export const obActorFor = O.fmapU(obIdentity, M.fmap(withIdentity));

export const obMainActor = O.fmapU(obActorFor, M.fmap(callWith(canisterId)));
